...
const AdminDashboard = () => {
    ...
    useEffect(() => {
        const q = collection(db, 'warga');
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            const wargaData: Warga[] = [];
            querySnapshot.forEach((doc) => {
                wargaData.push({ id: doc.id, ...doc.data() } as Warga);
            });

            setTotalWarga(wargaData.length);

            const lakiLaki = wargaData.filter(w => w.jenisKelamin === 'Laki-laki').length;
            const perempuan = wargaData.length - lakiLaki;
            setGenderData([
    ...
    return (
        <div className={styles.adminDashboard}>
            ...
            <div className={styles.fullWidthChartContainer}>
                 <h3>Kelompok Usia Sekolah</h3>
                 <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={schoolAgeData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="name" fontSize={12} />
                        <YAxis />
                        <Tooltip formatter={(value) => `${value} orang`} />
                        <Legend />
                        <Bar dataKey="jumlah" fill="#82ca9d" name="Jumlah Warga" />
                    </BarChart>
                 </ResponsiveContainer>
            </div>
        </div>
